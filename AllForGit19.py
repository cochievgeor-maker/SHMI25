...
def gap_to_wavenumber_and_color(gap_hartree):
    """
    Физически корректный перевод HOMO-LUMO gap:
      1) Hartree → eV
      2) eV → λ поглощения (1250/E)
      3) λ → RGB (видимая область)
      4) RGB → комплементарный цвет (цвет молекулы)
    """
    ...
    # Константы
    EV_PER_HARTREE = 27.211386245988  # точное значение
    ...
    # Цвет поглощения → видимый комплементарный
...
def wavelength_to_rgb_piecewise(wavelength_nm):
    """
    Перевод длины волны (нм) в RGB (0–255) для ВИДИМОГО спектра.
    Вне диапазона 380–780 нм возвращает (0, 0, 0) как цвет фотона.
    """
    ...
    # Коррекция яркости по краям спектра
    if wavelength < 420:
...
def complement_rgb(rgb):
    """
    Комплементарный цвет: то, что мы видим как цвет молекулы
    (поглощённое вычитается из белого света).
    """
    r, g, b = rgb
...
def get_color_with_extended_range(wavelength_nm):
    ...
    возвращает (rgb, info), как и старая версия.
    rgb — КОМПЛЕМЕНТАРНЫЙ цвет (видимый для глаза),
...
def gap_to_wavenumber_and_color(gap_hartree):
    """
    Физически осмысленный перевод HOMO–LUMO gap:
      1) Hartree → eV
      2) eV → λ поглощения
      3) λ → RGB(λ) как цвет поглощения
      4) комплементарный RGB → цвет молекулы
    Формат возвращаемого словаря сохранён, как в старой версии.
    """
    ...
    SPEED_OF_LIGHT_CM = 2.99792458e10    # см/с
    ...
    # Цвет молекулы (комплементарный к цвету поглощения)
